...
public class AddResultDao  {
	...
	public List showAll(String s){
		...
		String sql;
		if(s!=null & s!="")
			sql="select distinct(p.pathology_patient_id) from pathology_patient p,pathology_patient_test pt where p.pathology_patient_id=pt.pathology_patient and pt.test_status='pending' and p.patient_name like '"+s+"%'";
		else
			sql="select distinct(p.pathology_patient_id) from pathology_patient p,pathology_patient_test pt where p.pathology_patient_id=pt.pathology_patient and pt.test_status='pending'";
		try{
			Map map;
...
/*map.put("billid", rs.getInt("pathology_patient_id"));
				map.put("name", rs.getString("patient_name"));
				map.put("address", rs.getString("patient_address"));
				map.put("total", rs.getInt("total"));
				map.put("paid", rs.getInt("paid"));
				map.put("status", rs.getString("p_status"));
