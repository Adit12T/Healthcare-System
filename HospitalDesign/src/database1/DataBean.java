...
public class DataBean {
	...
	public boolean updatePathologyTestDetail(float tid, String tname,
	...
	public boolean savePathologyTestDetail(float tid, String tname,
			String tcontent, String srange, String erange, int tcost) {
		dbcon db = dbcon.getConnection();
		String sql = "insert into pathelogy_test_detail values('" + tid + "','"
				+ tname + "','" + tcontent + "','" + srange + "','" + erange
				+ "','" + tcost + "')";
		int flag = db.insertOrUpdate(sql);
		if (flag > 0) {
			return true;
	...
	public void deletePathologyTestDetail(float tid) {
		dbcon db = dbcon.getConnection();
	...
	public boolean savePathologyTestReport(int pid, String [] tid, String tdate,
		...
		for (int i = 0; i < tid.length; i++) {
			...
			String sql = "insert into pathology_test_report values(0,'" + pid
					+ "','" + tid[i] + "','" + DateConvertdata.convertDate(tdate) + "','" + result + "','" + refby
					+ "')";
	...
	public boolean saveTestResult(int rid,  String result) {
		// System.out.println();
		dbcon db = dbcon.getConnection();
		String sql = "UPDATE `pathology_test_report` SET  result = '" + result + "' WHERE `report_id`='" + rid + "'";
		int flag = db.insertOrUpdate(sql);
		if (flag > 0) {
			return true;
		}
