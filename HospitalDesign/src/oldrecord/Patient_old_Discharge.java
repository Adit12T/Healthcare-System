...
public class Patient_old_Discharge {
	...
	public List getOldPatient(String s) {
		...
		if (s != null && s != "") {
			...
			sql = "select * from patient p,discharge d where   d.patientid=p.patient_registrationno and  p.patient_name like '" + s
					+ "%' or p.patient_registrationno like '" + s + "%'";// or
																		// patient_registrationno
																		// like
																		// "+s+"%";
		} else
			sql = "select * from discharge d,patient p where d.patientid=p.patient_registrationno";
		...
		try {
			...
			while (rs.next()) {
				...
				map.put("admitdate",
						rs.getDate("admitdate"));
				
				map.put("dischargedate",
						rs.getDate("dischargedate"));
				map.put("totalamount",rs.getInt("totalamount"));
